...
trait FileSaver
{
    ...
    public function saveToPublic(\Illuminate\Http\UploadedFile $file, string $baseDir): array
    {
        $disk = 'public';
        $path = $file->store($baseDir, $disk);
        return [
            'disk'          => $disk,
            'path'          => $path,
            'size'          => $file->getSize(),
    ...
    */
    public function upload_file($uploaded_file, $model, $database_field_name, $base_path)
    {
        if ($uploaded_file) {
            ...
            // directory like: public/assets/uploads/categories/2025/
            $relativeDir = 'assets/uploads/' . $base_path . '/' . date('Y') . '/';
            ...
            // move file
            $uploaded_file->move($absoluteDir, $new_file_name);

            // save relative path in DB
            $model->update([$database_field_name => $relativeDir . $new_file_name]);
        }
    }
